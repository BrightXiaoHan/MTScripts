"""
This script is adapted from https://github.com/pytorch/fairseq/blob/master/examples/backtranslation/extract_bt_data.py
"""
import argparse
import fileinput
...
def main():
    ...
    parser.add_argument("--maxunk", type=int, help="max unk symbol in either src and tgt.")
    ...
    def validate(src, tgt):
        ...
        if ((args.minlen is not None and
            ...
            return False
        if args.maxunk is not None and tgt.count("<unk>") > args.maxunk:
            return False
        return True
    ...
    with open(args.output + "." + args.srclang,
              "w") as src_h, open(args.output + "." + args.tgtlang,
                                  "w") as tgt_h:
